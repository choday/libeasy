...
namespace ebase
{
	...
	public://实现者访问
		...
		event_emitter			private_on_want_write;//实现者继承,flush,end
	private:
	...
	public:
		...
		event_emitter	on_end;//可读缓冲区到末尾，以后不再可用,剩余数据可读取
		event_emitter	on_error;
        ebase::error    error;
	...
	public://实现者访问
		...
		void			private_set_event_executor( executor* event_executor );
		event_emitter	private_on_want_read;//实现者继承,pause,resume,read(empty)
	private:
